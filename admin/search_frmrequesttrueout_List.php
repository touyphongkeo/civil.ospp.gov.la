<html>
	<body>
    
 
             <div id='show' class="table-responsive p-10">
                <table class='table table-bordered' style="width: 250%">
                    <thead>
                    <tr align='center'>
                        <th>ລຳດັບ</th>
                        <th>ລະຫັດເອກະສານ</th>
                        <th>ເລກທີຂາອອກ</th>
                        <th>ວັນເດືອນປີອອກ</th>
                        <th>ເລກທີຂາເຂົ້າ</th>
                        <th>ຄຳຮ້ອງກ່ຽວກັບບັນຫາໃດ</th>
                        <th>ໂຈດ</th>
                        <th>ເຈົ້າຂອງຄຳຮ້ອງ</th>
                        <th>ບຸກຄົນທີ່ສາມ</th>
                        <th>ໜ່ວຍງານຮັບຜິດຊອບ</th>
                        <th>ພະນັກງານຮັບຜິດຊອບ</th>
                      </tr>
                </thead>
                   
                <?php
                include 'server/connect.php';
                $start=$_POST['start'];
                $end=$_POST['end'];
                $Item_ID=$_POST['Item_ID'];
                $In_No=$_POST['In_No'];
             
               
             
                if($start=="" or $end==""){$btw="";}
                else{ $btw="and Civ_ReQuestTrueOut.Item_Date between '$start' and '$end'";}

                if($Item_ID==""){$b="";}
                else{ $b="and (Civ_ReQuestTrueOut.Item_ID like N'%$Item_ID%' or Civ_ReQuestTrueOut.Item_ID like N'$Item_ID%')";}

                if($In_No==""){$d="";}
                else{ $d="and (Civ_ReQuestTrueOut.In_No like N'%$In_No%' or Civ_ReQuestTrueOut.In_No like N'$In_No%')";}

               

                $i=1;
                $sql = "SELECT * FROM Civ_ReQuestTrueOut where 1=1 $btw $b $d order by Civ_ReQuestTrueOut.item_Cnt asc";
                $query = sqlsrv_query( $conn, $sql );

                while($result = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC))
                {
                  $i++;
                ?>

                    <tbody>

                    <tr>
                   <td align='center'><?PHP echo $result["item_Cnt"]; ?></td>
                   <td align='center'><?PHP echo $result["Item_ID"]; ?></td>
                   <td align='center'><?PHP echo $result["Item_No"]; ?></td>
                   <?php
                   $date=$result["Item_Date"];
                   ?>
                   <td align='center'><?PHP echo date_format($date,'d/m/Y');?></td>
                   <td><?PHP echo $result["In_No"]; ?></td>
                   <td><?PHP echo $result["Request_Type"]; ?></td>
                   <td><?PHP echo $result["Civil"]; ?></td>
                   <td><?PHP echo $result["Request_Pers"]; ?></td>
                   <td><?PHP echo $result["CivilPers"]; ?></td>
                   <td><?PHP echo $result["Dept_Respond"]; ?></td>
                   <td><?PHP echo $result["Staff_Respond"]; ?></td>   
                  </tr>

                    </tbody>
                    <?php
                  }
                  ?>
                </table>
                </div>
	</body>
</html>
